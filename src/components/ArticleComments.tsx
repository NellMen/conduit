import React, {useEffect, useState} from "react";
import { requests } from "../api";
import { Comment } from "./Comment";
import {useAppSelector} from "../store/store";
import {ArticleCommentsProps, comments} from "./Types";


const ArticleComments: React.FC<ArticleCommentsProps> = ({slug, image}) => {
    const [loadingComments, setLoadingComments] = useState<boolean>(true);
    const [comments, setComments] = useState<comments[]>([]);
    const [postComValue, setPostComValue] = useState('');
    const [clicedPost, setClicedPost] = useState(false);
    const userNameOrig = useAppSelector(state => state.userInfoRequest.username);


    const clickPostComment = async (one: string, two: string) => {
        if (!userNameOrig) {
            alert("АВТОРИЗУЙСЯ")
            return
        }
        try {
            await requests.postComments(one, two);
            setClicedPost(true);
            setPostComValue('');
        } catch (e) {
            console.log(e);
        }
    }

    const handleDeleteComment = (commentId: number) => {
        if (confirm("Are you sure you want to delete this comment?")) {
            requests.deleteComment(slug, commentId);
            setClicedPost(true)
        }
    }

    useEffect(() => {
        async function getComments() {
            try {
                const comments = await requests.comments(slug);
                setComments(comments.data.comments);
                setLoadingComments(false);
                setClicedPost(false);
            } catch {
                console.log('error Internet');
                setLoadingComments(false);
            }
        }
        getComments();
    }, [clicedPost]);

    return (
        <div className="row">
            <div className="col-xs-12 col-md-8 offset-md-2">
                <form className="card comment-form">
                    <div className="card-block">
                        <textarea
                            className="form-control"
                            placeholder="Write a comment..."
                            rows={3}
                            value={postComValue}
                            onChange={(e) => {
                                setPostComValue(e.target.value);
                            }}
                        />
                    </div>
                    <div className="card-footer">
                        <img
                            src={image || 'data:image/jpeg;base64,'}
                            className="comment-author-img"
                        />
                        <a
                            href="#"
                            onClick={(e) => {
                                e.preventDefault();
                                clickPostComment(slug, postComValue);
                            }}
                            className="btn btn-sm btn-primary"
                        >
                            Post Comment
                        </a>
                    </div>
                </form>
                {loadingComments && 'Loading comments...'}
                {!loadingComments && comments.map((come) => (
                    <Comment
                        key={come.id}
                        onClickDeleteState={handleDeleteComment}
                        commentInfo={{ commentInfo: come }}
                    />
                ))}
            </div>
        </div>
    );
};

export default ArticleComments;

