import {Link} from "react-router-dom";
import React, {useEffect} from "react";
import {useAppSelector} from "../store/store";
import {requests} from "../api";
import {ProfileInfoProps} from "./Types";


const ProfileInfo: React.FC<ProfileInfoProps> = ( { usernameInfo } ) => {
    const userNameOrig = useAppSelector(state => state.userInfoRequest);
    const [isFavorited, setIsFavorited] = React.useState(usernameInfo.following);
    const [favoritesCount, setFavoritesCount] = React.useState<number>(0);

    useEffect(() => {
        if (usernameInfo.followersCount !== undefined){
            setFavoritesCount(usernameInfo.followersCount)
        }
        setIsFavorited(usernameInfo.following)
    }, [usernameInfo]);


    const clickFollow = async () => {
        if (!userNameOrig.username) {
            alert("ЗАЛОГИНЬСЯ")
        } else {
            try {
                if (!isFavorited) {
                    await requests.follow(usernameInfo.username);
                    setFavoritesCount(prev => prev + 1);
                } else {
                    await requests.Unfollow(usernameInfo.username);
                    setFavoritesCount(prev => prev - 1);
                }
                setIsFavorited(prev => !prev);
            } catch {
                return alert('Error');
            }
        }
    };

    return (
        <div className='container'>
            <div className='row'>
                <div className='col-xs-12 col-md-10 offset-md-1'>
                    <img
                        src={usernameInfo.image || 'data:image/jpeg;base64,'}
                        className='user-img'/>
                    <h4>{usernameInfo.username}</h4>
                    <p>
                        {usernameInfo.bio}
                    </p>
                    {(userNameOrig.username !== usernameInfo.username) && (
                        <button className='btn btn-sm btn-outline-secondary action-btn'
                         onClick={clickFollow}>
                            <i className='ion-plus-round'></i>
                            {(!isFavorited) ? 'Follow' : 'Unfollow'} {usernameInfo.username} ({favoritesCount})
                        </button>
                    )}

                    {(userNameOrig.username === usernameInfo.username) && (
                        <Link className='btn btn-sm btn-outline-secondary action-btn' to='/profile/settings'>
                        <i className='ion-gear-a'></i>Edit Profile Settings
                        </Link>
                    )}

                </div>
            </div>
        </div>
    )
}

export default ProfileInfo;
